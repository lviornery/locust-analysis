...
phicandidates = linspace(-5,5,10);
R2vec = zeros(size(phicandidates));
for i = 1:length(phicandidates)
    R2vec(i) = getNegR2(phicandidates(i));
end
plot(phicandidates,R2vec)

options = optimset('Display','iter')
offset = fmincon(@getNegR2,0,[],[],[],[],-30,10,[],options)

function avSE = getNegR2(offset)
    ...
    p = struct();
    p = params_static(p);
    p.ExtensorOffset = rad2deg(offset);
    p.dt = 1/2000;
    ...
    for indivIdx = 1:6
        ...
        for idx = 1:length(fileIndices)
            ...
            for i = 1:length(t)
                ...
                springArm = sin(phi(i) - p.ExtensorOffset);
        ...
        SEslopes(indivIdx) = sqrt((esqr/xosqr)*dof_deflator);
    end
    avSE = mean(SEslopes);
end